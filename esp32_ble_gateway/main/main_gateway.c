...
static void parse_sensor_data(uint8_t *data, uint16_t len, sensor_data_t *sensor_data)
{
    ...
    // Parse temperature (int16, big-endian)
    int16_t temp_raw = (data[0] << 8) | data[1];
    sensor_data->temperature = temp_raw / 100.0f;

    // Parse humidity (uint16, big-endian)
    uint16_t hum_raw = (data[2] << 8) | data[3];
    sensor_data->humidity = hum_raw / 100.0f;

    // Parse STM32 LED state
    sensor_data->led_state = data[4];
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_REG_EVT:
        ESP_LOGI(GATTC_TAG, "GATT client registered, status %d", param->reg.status);
        esp_err_t scan_ret = esp_ble_gap_set_scan_params(&ble_scan_params);
        if (scan_ret){
            ESP_LOGE(GATTC_TAG, "Set scan params error: %x", scan_ret);
        }
        break;
            ...
            if (count > 0){
                ...
                if (!char_elem_result){
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
                        ESP_LOGI(GATTC_TAG, "Characteristic 0x%04X found with NOTIFY", REMOTE_NOTIFY_CHAR_UUID);
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            ESP_LOGI(GATTC_TAG, "Notification registered");
            uint16_t count = 0;
            ...
            if (count > 0){
                ...
                if (!descr_elem_result){
                    ...
                    if (count > 0 && descr_elem_result[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG){
                        ESP_LOGI(GATTC_TAG, "Writing to CCCD to enable notification");
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
        if (param->scan_start_cmpl.status == ESP_BT_STATUS_SUCCESS) {
            ESP_LOGI(GATTC_TAG, "Scanning for %s", DEVICE_NAME);
        }
        break;
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (adv_name != NULL) {
                if (strlen(DEVICE_NAME) == adv_name_len && strncmp((char *)adv_name, DEVICE_NAME, adv_name_len) == 0 && !connect) {
                    ESP_LOGI(GATTC_TAG, "Found device: %s", DEVICE_NAME);
                    connect = true;
                    ESP_LOGI(GATTC_TAG, "Connecting...");
                    esp_ble_gap_stop_scanning();
                    
                    esp_ble_gatt_creat_conn_params_t conn_params = {
                        .remote_addr_type = scan_result->scan_rst.ble_addr_type,
                        .own_addr_type = BLE_ADDR_TYPE_PUBLIC,
                        .is_direct = true,
                    };
...
void app_main(void)
{
    ESP_LOGI(GATTC_TAG, "ESP32 BLE Gateway for ENV_SENSOR");
    ...
    ESP_LOGI(GATTC_TAG, "Target: %s, Service: 0x%04X, Char: 0x%04X", 
             DEVICE_NAME, REMOTE_SERVICE_UUID, REMOTE_NOTIFY_CHAR_UUID);
}